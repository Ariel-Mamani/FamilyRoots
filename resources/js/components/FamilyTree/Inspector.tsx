...
export default function Inspector({
    ...
    const { holidays } = useHolidays(countryCode, year);
    // Toast state
    ...
    const handleRememberSubmit = (e: React.FormEvent) => {
        ...
        // Verificar coincidencia con feriado (no bloqueante)
        let coincidenceToast = null;
        if (holidays && holidays.length > 0) {
            const md = `${rememberMonth.padStart(2, '0')}-${rememberDay.padStart(2, '0')}`;
            const found = holidays.find(h => h.date.slice(5) === md);
            if (found) {
                coincidenceToast = `¡Atención! La fecha coincide con el feriado: ${found.localName}`;
            }
        }
        if (coincidenceToast) {
            setToast(coincidenceToast);
            setTimeout(() => setToast(null), 3500);
        } else {
            setToast('Evento agregado al calendario');
    ...
    return (
        <>
            {toast && (
                <div className="fixed top-4 right-4 z-50 flex items-center gap-2 rounded-lg bg-yellow-400 p-4 shadow-lg text-white animate-in fade-in slide-in-from-top-5">
